		...
		protected override void Render(BaseView master) {
			...
			this.AddIteration(Modules, () => {
				modules.AddRow(row => {
					...
					var typeCell = row.AddColumnsLayout();
					typeCell.AddLabel()
						.BindCaption(Modules, module => module.Node.ProjectType);

					// Add count of properties in parentheses
					typeCell.AddLabel()
				...
				modules.AddRow(row => {
					...
					propertyTable.AddRow(propRow => {
						...
						propRow.AddLabel().SetCaption(("Action")).SetStyle(new Style { Width = new Length(150, In.Pixels) }); // Increased width for buttons column
					});
