...
import { sendEmail, EMAIL_RECIPIENTS } from '../services/emailService';
...
export const Contact = () => {
  ...
  const contactEmail = isB2B ? EMAIL_RECIPIENTS.B2B : EMAIL_RECIPIENTS.B2C;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Prepare the email content with all user information in French
      const fullName = `${formData.firstName} ${formData.lastName}`;
      const subject = formData.subject 
        ? formData.subject 
        : isB2B 
          ? `Demande de contact de ${fullName} (Professionnel)` 
          : `Demande de contact de ${fullName} (Particulier)`;
      
      // Simple French text format for the email content
      const content = `
Nom: ${formData.firstName}
Prénom: ${formData.lastName}
Email: ${formData.email}
Numéro de téléphone: ${formData.phone}
Type de client: ${isB2B ? 'Professionnel (B2B)' : 'Particulier (B2C)'}
...
Date d'envoi: ${new Date().toLocaleString('fr-FR')}
      ...
      // Send the email to the appropriate recipient based on client type
